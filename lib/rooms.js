...
Rooms.prototype._getRoomById = function(roomId) {
    return this._getRooms()[roomId];
};

//Rooms.prototype._getRoomBySocketId = function(socketId) {
//    var presentingInRoomIds = this._getRooms()[socketId].filter(function(id){
//        return id !== socketId;
//    });
//
//    return this._getRooms()[socketId];
//};

Rooms.prototype.has = function(roomId, socket) {
    var presentingInRoomIds = Object.keys(this._getRoomById(roomId));
    return presentingInRoomIds.indexOf(socket.id) !== -1;
};

Rooms.prototype.getRoomEmitterBySocketId = function(socketId) {
    var rooms = this._getRooms();
    for (var room_id in rooms) {
        if (rooms.hasOwnProperty(room_id)) {
            if (Object.keys(rooms[room_id]).indexOf(socketId)) {
                return this.getRoomEmitter(room_id);
            }
        }
    }
};
