...
var listen = function (server) {
    ...
    io.sockets.on('connection', function (client) {
        client.on('*', function (message) {

            var room = rooms.getRoomEmitterBySocketId(client.id);
            var eventName = message.data[0];
            var match = eventName.match(/^api:messages:([^:]+):update$/);
            if (match) {
                var id = match.pop();
                room.emit('api:messages:create', 'message recived');
        ...
        client.on('api:message:create', function (socket) {
